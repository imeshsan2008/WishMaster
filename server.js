  ...
  sock.ev.on('connection.update', async ({ connection }) => {
    if (connection === 'open') {
      ...
      try {
        ...
        let customMessage = getCustomMessage();

        for (const contact of birthdays) {
            customMessage = customMessage.replace('${name}', contact.name);

          let number = contact.phone.replace(/\D/g, '');
          if (!number.startsWith('94')) number = '94' + number.replace(/^0+/, '');
          ...
          await sock.sendMessage(jid, { image: profileBuffer, caption: customMessage });
