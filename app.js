...
function createOAuthClient() {
  ...
  return new google.auth.OAuth2(client_id, client_secret, (redirect_uris && redirect_uris[0]) || REDIRECT_URI);
}
...
async function send_festive_msg(sock, senderId) {
    try {
        ...
        const people = await listContacts_festive(auth);
        ...
        for (const contact of people) {
            ...
            while (attempts < maxRetries) {
                try {
                    ...
                    sendfastiveSave(name, contact.phones[0],false);
                }
            }
        }
    } catch (err) {
...
sock.ev.on("messages.upsert", async (msgData) => {
  try {
    ...
    switch (command) {
      ...
      case ".send":
        await sock.sendMessage(sender, {
          text: `⏳ Checking for today's birthdays...\n\n> Wish Master V1.0 | Command`,
        });

        await sendTodaysBirthdays(sock, sender);

        await sock.sendMessage(sender, {
          text: `✅ Birthday check completed.\n\n> Wish Master V1.0 | Command`,
        });
      ...
      case ".menu":
        await sock.sendMessage(sender, {
          text:
            ...
            `*This bot sends birthday wishes automatically.*\n\n` +
            `> WishMaster v1.0`,
        });
        addReaction(sock, message.key, "📃");
        break;
...
app.get('/api/festive-logs', (req, res) => {
  try {
    if (!fs.existsSync("festiveData.json")) return res.json([]);
    const logs = JSON.parse(fs.readFileSync("festiveData.json", 'utf8'));
    res.json(logs);
  } catch (err) {
...
function sendfastiveSave(contactName, contactNumber, success) {
  let data = {};

  // Load existing file
  if (fs.existsSync("festiveData.json")) {
    try {
      data = JSON.parse(fs.readFileSync("festiveData.json", "utf8"));
    } catch (err) {
  ...
  fs.writeFileSync("festiveData.json", JSON.stringify(data, null, 2));
}
...
app.get('/sendfestive', async (req, res) => {
  try {
    ...
    res.json({ ok: true, message: "✅ Festive image will be sent shortly!" });
    ...
    res.status(500).json({ ok: false, message: "Error sending festive image" });
  }
});
